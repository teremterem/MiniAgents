...
async def main():
    ...
    async for msg_promise in reply_sequence:
        ...
        # Let's separate messages with a double newline (even though in
...
Here is how you can implement a dialog loop between an agent and a user from ground up yourself (for simplicity there is no history agent in this example - checkout `in_memory_history_agent` and how it is used if you want to know how to implement your own history agent too):
...
The `AWAIT` sentinel is used in the example above to ensure that the `user_agent` completes its execution before the `assistant_agent` starts. This is necessary because the `user_agent` needs to print the assistant's response and then wait for user input before the `assistant_agent` can process that input.

### 🔄 Dialog Loop and History Agents

The `dialog_loop` agent is a pre-packaged agent that facilitates a conversation between a user and an AI assistant. It manages the flow of messages between the user, the assistant, and optionally, a history agent.

The `markdown_history_agent` is a history agent that writes the chat history to a markdown file. By default, it writes to a file named `CHAT.md` in the current working directory, but this can be customized.
...
- `user_agent`: A basic user agent that reads from and writes to the console.
- `in_memory_history_agent`: Keeps the chat history in memory.

You're encouraged to explore the source code in the `miniagents.ext` package to see how these various agents are implemented. This can give you insights into how to create your own custom agents or modify existing ones to suit your needs.
...
As long as the global `start_asap` setting is set to `True` (which is the default - see the source code of `Promising`, the parent class of `MiniAgents` context manager for details), the actual agent functions will start processing at the earliest task switch (the behaviour of `asyncio.create_task()`, which is used under the hood). In this example it is going to be at the beginning of the first iteration of the `async for` loop inside the `main` function.
...
Here's a simple example demonstrating how to use `miniagent.start_inquiry()` and then do `.send_message()` two times before calling `.reply_sequence()` (instead of all-in-one `miniagents.inquire()`):
...
The difference between these message types is in the default values of the `role` field of the message. `UserMessage` has a role of "user", `SystemMessage` has a role of "system", and `AssistantMessage` has a role of "assistant".
