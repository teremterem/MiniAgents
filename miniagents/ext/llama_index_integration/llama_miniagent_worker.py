...
class LlamaMiniAgentWorker(BaseAgentWorker):
    ...
    def from_tools(
        ...
        **kwargs: Any,
        ...
        if not isinstance(llm, LlamaMiniAgentLLM):
            raise ValueError("llm must be a LlamaMiniAgentLLM instance")

        if callback_manager is not None:
            llm.callback_manager = callback_manager

        if not llm.metadata.is_function_calling_model:
            raise ValueError(f"Model name {llm.model} does not support function calling API. ")
    ...
    def get_latest_tool_calls(self, task: Task) -> Optional[List[OpenAIToolCall]]:
        ...
        return chat_history[-1].additional_kwargs.get("tool_calls", None) if chat_history else None
    ...
    def _get_llm_chat_kwargs(
        ...
        if openai_tools:
            llm_chat_kwargs.update(tools=openai_tools, tool_choice=resolve_tool_choice(tool_choice))
        return llm_chat_kwargs

    def _process_message(self, task: Task, chat_response: ChatResponse) -> AgentChatResponse:
        ai_message = chat_response.message
        task.extra_state["new_memory"].put(ai_message)
        return AgentChatResponse(response=str(ai_message.content), sources=task.extra_state["sources"])

    def _get_stream_ai_response(self, task: Task, **llm_chat_kwargs: Any) -> StreamingAgentChatResponse:
    ...
    async def _get_async_stream_ai_response(self, task: Task, **llm_chat_kwargs: Any) -> StreamingAgentChatResponse:
        chat_stream_response = StreamingAgentChatResponse(
    ...
    def _call_function(
        ...
        with self.callback_manager.event(
            ...
            payload={
                ...
                EventPayload.TOOL: (
                    tool.metadata if tool is not None else ToolMetadata(description="", name=function_name)
                ),
            },
        ) as event:
            ...
            if error_message is not None:
                ...
                tool_output = build_error_tool_output(function_name, function_args_str, error_message)
            else:
                tool_output = call_tool_with_error_handling(tool, argument_dict, error_message=error_message)
        ...
        return tool.metadata.return_direct and not tool_output.is_error if tool is not None else False
    ...
    def _should_continue(self, tool_calls: Optional[List[OpenAIToolCall]], n_function_calls: int) -> bool:
        ...
        if not self._should_continue(latest_tool_calls, task.extra_state["n_function_calls"]):
            ...
            for tool_call in latest_tool_calls:
                ...
                if return_direct and len(latest_tool_calls) == 1:
                    ...
                    chat_response = ChatResponse(message=ChatMessage(role=MessageRole.ASSISTANT, content=response_str))
    ...
    async def _arun_step(
        ...
        llm_chat_kwargs = self._get_llm_chat_kwargs(task, openai_tools, tool_choice)
        agent_chat_response = await self._get_async_agent_response(task, mode=mode, **llm_chat_kwargs)
    ...
    def run_step(self, step: TaskStep, task: Task, **kwargs: Any) -> TaskStepOutput:
        ...
        return self._run_step(step, task, mode=ChatResponseMode.WAIT, tool_choice=tool_choice)

    @trace_method("run_step")
    async def arun_step(self, step: TaskStep, task: Task, **kwargs: Any) -> TaskStepOutput:
        """Run step (async)."""
        tool_choice = kwargs.get("tool_choice", "auto")
        return await self._arun_step(step, task, mode=ChatResponseMode.WAIT, tool_choice=tool_choice)
    ...
    def stream_step(self, step: TaskStep, task: Task, **kwargs: Any) -> TaskStepOutput:
        ...
        tool_choice = kwargs.get("tool_choice", "auto")
        return self._run_step(step, task, mode=ChatResponseMode.STREAM, tool_choice=tool_choice)

    @trace_method("run_step")
    async def astream_step(self, step: TaskStep, task: Task, **kwargs: Any) -> TaskStepOutput:
        """Run step (async stream)."""
        tool_choice = kwargs.get("tool_choice", "auto")
        return await self._arun_step(step, task, mode=ChatResponseMode.STREAM, tool_choice=tool_choice)
